...
def _sse_handler_factory(client_queue: queue.Queue) -> Any:
...
def _sse_serialize_event(event: dict[str, Any]) -> bytes | None:
    try:
...
def _make_handler(emitter: DashboardEmitter) -> type[BaseHTTPRequestHandler]:
    class DashboardHandler(BaseHTTPRequestHandler):
        ...
        def _serve_sse(self) -> None:
            ...
            handler = _sse_handler_factory(client_queue)
            ...
            try:
                while True:
                    ...
                    payload = _sse_serialize_event(event)
                    if payload is None:
                        continue
