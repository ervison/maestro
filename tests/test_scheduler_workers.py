...
def test_worker_blocks_write_outside_workdir():
    """Worker path guard prevents writes outside workdir."""
    # This test verifies that tool execution inside the worker respects path guard
    # The actual guard is in execute_tool, but we verify the workdir is resolved
    with patch("maestro.multi_agent._run_agentic_loop") as mock_loop:
        mock_loop.return_value = "Task completed"

        # Create a temp directory as workdir
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check workdir was resolved and passed correctly
            mock_loop.assert_called_once()
            call_kwargs = mock_loop.call_args.kwargs
            workdir_arg = call_kwargs["workdir"]
            assert isinstance(workdir_arg, Path)
            assert workdir_arg.resolve() == Path(tmpdir).resolve()
