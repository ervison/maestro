...
def test_agentic_loop_streaming_deltas_not_duplicated(tmp_path):
...
def test_agentic_loop_preserves_tool_call_context(tmp_path):
...
def test_agentic_loop_uses_final_message_when_no_deltas(tmp_path):
