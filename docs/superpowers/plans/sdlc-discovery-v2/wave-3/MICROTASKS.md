# Wave 3 — Reviewer Module

## Progress

| Microtask | Description | Status |
|-----------|-------------|--------|
| 3.1 | Create `maestro/sdlc/reviewer.py` | `[ ]` |
| 3.2 | Create `tests/test_sdlc_reviewer.py` | `[ ]` |
| 3.3 | Commit Wave 3 | `[ ]` |
...
**Goal:** Create `maestro/sdlc/reviewer.py` — the LLM-based gate validator that runs after each
sprint and evaluates cross-artifact consistency — and write its full test suite.

**Why this wave third:** The `Reviewer` class depends on `GateResult` (defined in Wave 2) and
`SDLCArtifact` (existing).  The harness (Wave 4) will import `Reviewer`; it must exist first.

**Dependencies:** Wave 1 complete, Wave 2 complete.

**Files touched:**
- `maestro/sdlc/reviewer.py` (create)
- `tests/test_sdlc_reviewer.py` (create)

---

## `[ ]` Microtask 3.1 — Create `maestro/sdlc/reviewer.py`

**File:** `maestro/sdlc/reviewer.py` (create new)

**Pre-condition:** `GateResult` and `SDLCArtifact` exist in `maestro/sdlc/schemas.py`.
...
**Verification:** Run `python -c "from maestro.sdlc.reviewer import Reviewer; print('OK')"` — prints `OK`.

---

## `[ ]` Microtask 3.2 — Create `tests/test_sdlc_reviewer.py`

**File:** `tests/test_sdlc_reviewer.py` (create new)

**Pre-condition:** Microtask 3.1 must be applied.
...
**Verification:** Run `pytest tests/test_sdlc_reviewer.py -v` — ALL PASS.

---

## `[ ]` Microtask 3.3 — Commit Wave 3

**Pre-condition:** All tests in `tests/test_sdlc_reviewer.py` pass.
